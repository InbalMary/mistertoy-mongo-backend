...
export async function addReview(req, res) {
    ...
    try {
        ...
        if (!toyId) {
            return res.status(400).send({ err: 'Missing toyId' })
        }

        if (!loggedinUser) {
            return res.status(401).send({ err: 'User must be logged in' })
        }
        ...
        loggedinUser.score = (loggedinUser.score || 0) + 10
        ...
        // review.aboutUser = await userService.getById(toyId)
